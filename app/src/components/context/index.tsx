...
export const UserProvider = ({ children }: UserContextProps) => {
  ...
  const fetchUser = async () => {
    try {
      ...
      console.log(pathname,'pathname ishere')

      if (!token) {
        if (pathname !== '/login' && pathname !== '/register' && !pathname.includes('/reset-password' )) 
        {
          router.push('/login');
          return;
        }
      ...
      if (response.ok) {
        ...
        if (pathname == '/login' || pathname == '/register' || pathname=='resetpage') 
       {
          router.push('/');
        } else {
